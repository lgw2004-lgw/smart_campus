  ...
  <div>
    <el-card shadow="never" style="border-radius:14px;background:linear-gradient(135deg,#1e5eff,#5b8cff);color:#fff">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div><div style="font-size:13px;opacity:.9">我的宿舍</div><div style="font-size:20px;font-weight:800;margin-top:4px">{{ assign ? `${assign.building_name || assign.building_id} · ${assign.room_no || assign.room_id} · ${assign.bed_no}号床` : '尚未分配宿舍' }}</div><div style="font-size:12px;opacity:.85;margin-top:4px">与管理端宿舍管理实时联动 · 已住/容量实时更新</div></div>
    ...
    <div style="margin-top:14px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
      ...
      <div style="display:flex;gap:8px;align-items:center">
        <el-select v-model="selectedShuyuan" placeholder="选择书院" clearable style="width:180px" @change="loadRooms">
          <el-option v-for="s in shuyuanOptions" :key="s" :label="s" :value="s" />
        </el-select>
    ...
    <div class="dorm-grid">
      <div class="dorm-card" v-for="r in rooms" :key="r.room_id" :class="{selected: selected?.room_id===r.room_id, full: r.occupied>=r.capacity || !isPublishOpen}" @click="isPublishOpen && (selected=r)">
        ...
        <div style="font-size:12px;color:#5a6b8a;margin-top:4px">书院：{{ getShuyuan(buildingMap[r.building_id]?.building_name) }}</div>
...
const selectedShuyuan=ref<string|null>(null)
const shuyuanOptions=ref<string[]>([])
...
function getShuyuan(name:string){ if(!name) return '-'; const m=name.match(/(.+?住宿书院)/); return m?m[1]:name }
...
async function loadMeta(){
  try{
    ...
    for(const x of (b.data.list||[])){ const s=getShuyuan(x.building_name); if(s) set.add(s) }
    shuyuanOptions.value=Array.from(set)
  }catch{}
  try{
...
async function loadRooms(){
  ...
  if(selectedShuyuan.value) data.shuyuan=selectedShuyuan.value
...
async function loadAssign(){
  try{
    const r:any=await request.get('/dorm/queryAssign', {params:{studentId:sid}})
    if(r.data){
      assign.value={building_id:r.data.building_id, building_name:r.data.building_name, room_id:r.data.room_id, room_no:r.data.room_no, bed_no:r.data.bed_no}
    }else assign.value=null
  }catch{ assign.value=null }
}
async function doAssign(){
  ...
  try{
    ...
    ElMessage.success('选宿舍成功'); await loadAssign(); loadRooms()
...
async function doExchange(){
  try{
    ...
    ElMessage.success('调宿成功'); await loadAssign()
...
onMounted(async()=>{ await loadMeta(); await loadRooms(); await loadAssign() })
</script>
<style scoped>
